...
interface LogisticsOverlayViewProps {
  venueId: string | null
  onEventClick?: (event: Event) => void
}

export function LogisticsOverlayView({ venueId, onEventClick }: LogisticsOverlayViewProps) {
  ...
  const totalCapacityUsed = useMemo(() => {
    ...
    stages.forEach((s) => (byStage[s.id] = s.legal_capacity ?? s.comfort_capacity ?? s.capacity ?? 0))
    let total = 0
    upcomingEvents.forEach((ev) => {
      const sid = ev.stageId ?? ev.linkedStageIds?.[0]
      if (sid && byStage[sid]) total += byStage[sid]
    })
    return total
  }, [stages, upcomingEvents])
