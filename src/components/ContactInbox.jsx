...
import { 
  Mail, X, CheckCircle, Archive, Reply, 
  ...
  const markAsRead = async (id) => {
    ...
    try {
      ...
      if (selectedSubmission?.id === id) {
        setSelectedSubmission({ ...selectedSubmission, status: 'read', read_at: new Date().toISOString() })
      }
    } catch (error) {
                ...
                <div className="divide-y divide-gray-800">
                  {filteredSubmissions.map((submission) => (
                    ...
                    >
                      ...
                      <div className="mt-2">
                        <span className={`px-2 py-0.5 rounded text-xs border ${getStatusColor(submission.status)}`}>
                          {submission.status}
                        </span>
                      </div>
                    </button>
              ...
              <div className="flex-1 flex flex-col overflow-y-auto">
                <div className="p-6 border-b border-gray-800">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span className={`px-3 py-1 rounded text-sm border ${getStatusColor(selectedSubmission.status)}`}>
