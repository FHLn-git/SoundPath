...
const ConfirmationModal = ({
  ...
  const criticalStage =
    ...
    (track?.column && ['team-review', 'contracting', 'upcoming'].includes(track.column))
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div
              className={`bg-gray-900 rounded-lg p-6 w-full max-w-md border-2 backdrop-blur-sm ${
                criticalStage && action === 'reject' ? 'border-red-500/80' : 'border-gray-800'
              }`}
            >
              <div className="flex items-center gap-3 mb-4">
                <div
                  className={`p-2 rounded-lg ${
                    criticalStage && action === 'reject' ? 'bg-red-500/20' : 'bg-yellow-500/20'
                  }`}
                >
                  <AlertTriangle
                    size={24}
                    className={
                      criticalStage && action === 'reject' ? 'text-red-400' : 'text-yellow-400'
                    }
                  />
                </div>
                <h2
                  className={`text-2xl font-bold ${
                    criticalStage && action === 'reject' ? 'text-red-400' : 'text-white'
                  }`}
                >
              ...
              {criticalStage && action === 'reject' && (
                <div className="mb-4 p-4 bg-red-500/20 border-2 border-red-500/60 rounded-lg">
                  ...
                  <p className="text-red-300 text-sm">
                    You are about to reject a track in a <strong>vital business stage</strong>. This
                    action will move the track to Archive and cannot be easily undone.
                  </p>
                </div>
              )}
                ...
                <div>
                  ...
                  <p
                    className={`font-semibold ${
                      criticalStage && action === 'reject' ? 'text-red-400' : 'text-gray-300'
                    }`}
                  >
                    {getActionText()}
                  </p>
                    ...
                    <textarea
                      ...
                      placeholder={
                        requireRejectionReason
                          ? 'Please provide a reason for rejection (minimum 5 characters)'
