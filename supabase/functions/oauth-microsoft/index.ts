// OAuth: Microsoft 365 (Calendar + basic profile via Microsoft Graph)
...
async function aesGcmEncrypt(plaintext: string, keyBytes: Uint8Array) {
  ...
  return `v1:${b64urlEncode(iv)}:${b64urlEncode(ctBytes)}`
}
...
serve(async (req) => {
  ...
  const redirectUri = new URL('/functions/v1/oauth-microsoft', supabaseUrl)
  ...
  if (action === 'start') {
    ...
    if (!orgId || !returnTo) return json(400, { error: 'organization_id and return_to are required' })
    ...
    if (!clientId) return json(500, { error: 'Missing MICROSOFT_OAUTH_CLIENT_ID' })
  ...
  if (action === 'callback') {
    ...
    if (oauthError) {
      return json(400, { error: oauthError })
    }
    if (!code || !state || !state.includes('.')) return json(400, { error: 'Missing code/state' })
    if (!stateSecret) return json(500, { error: 'Missing OAUTH_STATE_SECRET' })
    ...
    if (!payload?.org_id || !payload?.return_to || !payload?.code_verifier) return json(400, { error: 'Invalid state payload' })

    const clientId = Deno.env.get('MICROSOFT_OAUTH_CLIENT_ID') || ''
    const clientSecret = Deno.env.get('MICROSOFT_OAUTH_CLIENT_SECRET') || ''
    if (!clientId || !clientSecret) return json(500, { error: 'Missing Microsoft OAuth client config' })
    ...
    if (!tokenRes.ok) {
      return new Response(null, {
        status: 302,
        headers: {
          Location: `${payload.return_to}?integration=microsoft&connected=0&error=${encodeURIComponent(tokenJson?.error_description || 'token_exchange_failed')}`,
        },
      })
    }
    ...
    const meRes = await fetch(ME_URL, { headers: { Authorization: `${tokenType} ${accessToken}` } })
    ...
    const redirect = upsertError
      ? `${payload.return_to}?integration=microsoft&connected=0&error=${encodeURIComponent(upsertError.message)}`
      : `${payload.return_to}?integration=microsoft&connected=1`
